...
local function validate_augments(catalysts, sources)
	...
	for _, inst in ipairs(sources) do
		...
		if not item or as_number(item:ID()) == 0 or as_number(item:AugType()) == 0 then
			return nil, "Every hand-in must be an augment."
		end
...
local function fuse_augments(sources, source_ids)
	...
	local source_name = output:GetCustomData("augs_in_augs_root_name")
	if source_name == "" then
	...
	output:SetCustomData("augs_in_augs_fused", "1")
	output:SetCustomData("augs_in_augs_root_name", source_name)
	output:SetCustomData("augs_in_augs_component_count", tostring(#sources))
	output:SetCustomData("augs_in_augs_sources", table.concat(source_ids, ","))
...
function event_say(e)
	...
	e.self:Say("Hand me one Augment Catalyst plus one to three augment items and no coins. I will return one fused augment that carries the combined supported stats from the augment items.")
end
...
function event_trade(e)
	...
	if not e.self:CheckHandin(e.other, {}, validation.required, {}) then
		e.self:Say("The hand-in did not line up cleanly. Try handing me only the shards you want fused.")
		item_lib.return_items(e.self, e.other, e.trade)
		return
	end

	e.other:PushItemOnCursor(result.item)
