...
static DWORD GetCXStr(PCXSTR pCXStr, PCHAR szBuffer, DWORD maxlen)
{
    if (!szBuffer) {
        return 0;
    }
    ...
    if (pCXStr->Encoding == 0) {
        if (pCXStr->Length < maxlen) {
            strcpy_s(szBuffer, maxlen, pCXStr->Text);
            return pCXStr->Length;
        }

        strncpy_s(szBuffer, maxlen, pCXStr->Text, _TRUNCATE);
        return maxlen;
    }
    ...
    DWORD output = 0;
    maxlen--;
    while (pCXStr->Text[input] && output < maxlen) {
        szBuffer[output++] = pCXStr->Text[input];
        input += 2;
    }
    szBuffer[output] = 0;
    return output;
...
// Native GetChildItem can miss controls that are identifiable only by ScreenID.
