...
// ---------------------------------------------------------------------------
// UI size presets. The XML ships Small/Large clones of every Advanced Loot
// screen (generated by tools/generate-ui-scales.ps1) whose item names carry a
// _S/_L suffix; the active preset decides which screen and child names the
// windows resolve, and scales the geometry the DLL drives at runtime (forced
// list row heights, pooled control sizes). Persisted per install in
// native_ui_scale.ini next to eqgame.exe.
// ---------------------------------------------------------------------------

enum NativeUiScalePreset
{
	kNativeUiScaleSmall = 0,
	kNativeUiScaleMedium,
	kNativeUiScaleLarge
};

static int gNativeUiScale = kNativeUiScaleMedium;
static bool gNativeUiScaleLoaded = false;
static bool gNativeUiScalePendingRecreate = false;

static void NativeUiScaleConfigPath(char* path, size_t path_size)
{
	if (gszEQPath[0]) {
		sprintf_s(path, path_size, "%s\\native_ui_scale.ini", gszEQPath);
	}
	else {
		strcpy_s(path, path_size, "native_ui_scale.ini");
	}
}

static void NativeUiScaleLoad()
{
	if (gNativeUiScaleLoaded) {
		return;
	}

	gNativeUiScaleLoaded = true;
	char path[MAX_PATH];
	NativeUiScaleConfigPath(path, sizeof(path));
	...
	fclose(file);
	if (first == 'S' || first == 's') {
		gNativeUiScale = kNativeUiScaleSmall;
	}
	else if (first == 'L' || first == 'l') {
		gNativeUiScale = kNativeUiScaleLarge;
	}
}

static void NativeUiScaleSave()
{
	char path[MAX_PATH];
	NativeUiScaleConfigPath(path, sizeof(path));
	...
	fputc(gNativeUiScale == kNativeUiScaleSmall ? 'S' : gNativeUiScale == kNativeUiScaleLarge ? 'L' : 'M', file);
	fclose(file);
}

static const char* NativeUiScaleSuffix()
{
	NativeUiScaleLoad();
	return gNativeUiScale == kNativeUiScaleSmall ? "_S" :
		gNativeUiScale == kNativeUiScaleLarge ? "_L" : "";
}

static const char* NativeUiScaleLabel()
{
	NativeUiScaleLoad();
	return gNativeUiScale == kNativeUiScaleSmall ? "Small" :
		gNativeUiScale == kNativeUiScaleLarge ? "Large" : "Medium";
}

static int NativeUiScalePercent()
{
	NativeUiScaleLoad();
	return gNativeUiScale == kNativeUiScaleSmall ? 80 :
		gNativeUiScale == kNativeUiScaleLarge ? 125 : 100;
}

// Matches the generator's rounding: floor(value * percent / 100 + 0.5).
static int NativeUiScaled(int value)
{
...
{
	if (!shared) {
		return NativeUiScaled((column == kAALPersonalLoot || column == kAALPersonalLeave || column == kAALPersonalNever) ? 26 : 24);
	}

	return NativeUiScaled((column == kAALSharedStatus || column == kAALSharedAction || column == kAALSharedManage) ? 26 : 24);
}
...
public:
	NativeAutoLootWnd() : CCustomWnd((char*)NativeUiSized("NativeAutoLootWnd"))
	{
		...
		PersonalLabel = GetChildItem(NativeUiSized("AALW_PersonalLabel"));
		SetAllLabel = GetChildItem(NativeUiSized("AALW_SetAllLabel"));
		SharedLabel = GetChildItem(NativeUiSized("AALW_SharedLabel"));
		PersonalList = (CListWnd*)GetChildItem(NativeUiSized("AALW_PersonalList"));
		SharedList = (CListWnd*)GetChildItem(NativeUiSized("AALW_SharedList"));
		StatusLabel = GetChildItem(NativeUiSized("AALW_StatusLabel"));
		MasterLabel = GetChildItem(NativeUiSized("AALW_MasterLabel"));
		RuleSummaryLabel = GetChildItem(NativeUiSized("AALW_RuleSummaryLabel"));
		RefreshButton = (CButtonWnd*)GetChildItem(NativeUiSized("AALW_RefreshButton"));
		EditFiltersButton = (CButtonWnd*)GetChildItem(NativeUiSized("AALW_EditFiltersButton"));
		LootSettingsButton = (CButtonWnd*)GetChildItem(NativeUiSized("AALW_LootSettingsButton"));
		LootAllButton = (CButtonWnd*)GetChildItem(NativeUiSized("AALW_LootAllButton"));
		LeaveAllButton = (CButtonWnd*)GetChildItem(NativeUiSized("AALW_LeaveAllButton"));
		LootButton = (CButtonWnd*)GetChildItem(NativeUiSized("AALW_LootButton"));
		LeaveButton = (CButtonWnd*)GetChildItem(NativeUiSized("AALW_LeaveButton"));
		AlwaysButton = (CButtonWnd*)GetChildItem(NativeUiSized("AALW_AlwaysButton"));
		NeverButton = (CButtonWnd*)GetChildItem(NativeUiSized("AALW_NeverButton"));
		NeedButton = (CButtonWnd*)GetChildItem(NativeUiSized("AALW_NeedButton"));
		GreedButton = (CButtonWnd*)GetChildItem(NativeUiSized("AALW_GreedButton"));
		NoButton = (CButtonWnd*)GetChildItem(NativeUiSized("AALW_NoButton"));
		AlwaysNeedButton = (CButtonWnd*)GetChildItem(NativeUiSized("AALW_AlwaysNeedButton"));
		AlwaysGreedButton = (CButtonWnd*)GetChildItem(NativeUiSized("AALW_AlwaysGreedButton"));
		AskButton = (CButtonWnd*)GetChildItem(NativeUiSized("AALW_AskButton"));
		RollButton = (CButtonWnd*)GetChildItem(NativeUiSized("AALW_RollButton"));
		FreeGrabButton = (CButtonWnd*)GetChildItem(NativeUiSized("AALW_FreeGrabButton"));
		GiveButton = (CButtonWnd*)GetChildItem(NativeUiSized("AALW_GiveButton"));
		ManageButton = (CButtonWnd*)GetChildItem(NativeUiSized("AALW_ManageButton"));
		LeaveCorpseButton = (CButtonWnd*)GetChildItem(NativeUiSized("AALW_LeaveCorpseButton"));
		ApplyFiltersCheck = (CButtonWnd*)GetChildItem(NativeUiSized("AALW_ApplyFiltersCheck"));
		GroupedByNpcCheck = (CButtonWnd*)GetChildItem(NativeUiSized("AALW_GroupedByNpcCheck"));
		...
		for (int pool_row = 0; pool_row < kAALPoolPersonalRows; ++pool_row) {
			...
			PersonalIconPool[pool_row] = (CButtonWnd*)GetChildItem(NativeUiSized(name));
		...
		for (int pool_row = 0; pool_row < kAALPoolSharedRows; ++pool_row) {
			...
			SharedIconPool[pool_row] = (CButtonWnd*)GetChildItem(NativeUiSized(name));
		...
		for (int pool_row = 0; pool_row < kAALPoolPersonalRows; ++pool_row) {
			for (int slot = 0; slot < kAALPoolPersonalCols; ++slot) {
				...
				PersonalPool[pool_row][slot] = (CButtonWnd*)GetChildItem(NativeUiSized(name));
		...
		for (int pool_row = 0; pool_row < kAALPoolSharedRows; ++pool_row) {
			for (int slot = 0; slot < kAALPoolSharedCols; ++slot) {
				...
				SharedPool[pool_row][slot] = (CButtonWnd*)GetChildItem(NativeUiSized(name));
...
{
public:
	NativeAutoLootRulesWnd() : CCustomWnd((char*)NativeUiSized("NativeAutoLootRulesWnd"))
	{
		CloseOnESC = 1;
		SetWndNotification(NativeAutoLootRulesWnd);

		SummaryLabel = GetChildItem(NativeUiSized("AALR_SummaryLabel"));
		StatusLabel = GetChildItem(NativeUiSized("AALR_StatusLabel"));
		RuleList = (CListWnd*)GetChildItem(NativeUiSized("AALR_RuleList"));
		RefreshButton = (CButtonWnd*)GetChildItem(NativeUiSized("AALR_RefreshButton"));
		ShareButton = (CButtonWnd*)GetChildItem(NativeUiSized("AALR_ShareButton"));
		SearchEdit = (CEditWnd*)GetChildItem(NativeUiSized("AALR_SearchEdit"));
		ClearSearchButton = (CButtonWnd*)GetChildItem(NativeUiSized("AALR_ClearSearchButton"));
		TglEnabled = (CButtonWnd*)GetChildItem(NativeUiSized("AALR_TglEnabled"));
		TglSplit = (CButtonWnd*)GetChildItem(NativeUiSized("AALR_TglSplit"));
		TglConfirm = (CButtonWnd*)GetChildItem(NativeUiSized("AALR_TglConfirm"));
		TglLore = (CButtonWnd*)GetChildItem(NativeUiSized("AALR_TglLore"));
		TglShow = (CButtonWnd*)GetChildItem(NativeUiSized("AALR_TglShow"));
		TglLootAll = (CButtonWnd*)GetChildItem(NativeUiSized("AALR_TglLootAll"));

		for (int pool_row = 0; pool_row < kAALRulePoolRows; ++pool_row) {
			char name[48];
			for (int slot = 0; slot < 4; ++slot) {
				sprintf_s(name, "AALR_CB_R%dC%d", pool_row, slot);
				CheckPool[pool_row][slot] = (CButtonWnd*)GetChildItem(NativeUiSized(name));
			...
			RemovePool[pool_row] = (CButtonWnd*)GetChildItem(NativeUiSized(name));
			if (RemovePool[pool_row]) {
				((CXWnd*)RemovePool[pool_row])->Show(0, 1);
			}

			sprintf_s(name, "AALR_IB_R%d", pool_row);
			IconPool[pool_row] = (CButtonWnd*)GetChildItem(NativeUiSized(name));
...
{
public:
	NativeAutoLootSettingsWnd() : CCustomWnd((char*)NativeUiSized("NativeAutoLootSettingsWnd"))
	{
		CloseOnESC = 1;
		SetWndNotification(NativeAutoLootSettingsWnd);

		SummaryLabel = GetChildItem(NativeUiSized("AALS_SummaryLabel"));
		GroupSummaryLabel = GetChildItem(NativeUiSized("AALS_GroupSummaryLabel"));
		StatusLabel = GetChildItem(NativeUiSized("AALS_StatusLabel"));
		AutoLootCheck = (CButtonWnd*)GetChildItem(NativeUiSized("AALS_AutoLootCheck"));
		NeedGreedCheck = (CButtonWnd*)GetChildItem(NativeUiSized("AALS_NeedGreedCheck"));
		AutoShowCheck = (CButtonWnd*)GetChildItem(NativeUiSized("AALS_AutoShowCheck"));
		ShowNewCheck = (CButtonWnd*)GetChildItem(NativeUiSized("AALS_ShowNewCheck"));
		ConfirmRemoveCheck = (CButtonWnd*)GetChildItem(NativeUiSized("AALS_ConfirmRemoveCheck"));
		AutoRemoveLoreCheck = (CButtonWnd*)GetChildItem(NativeUiSized("AALS_AutoRemoveLoreCheck"));
		RefreshButton = (CButtonWnd*)GetChildItem(NativeUiSized("AALS_RefreshButton"));
		GroupNoneButton = (CButtonWnd*)GetChildItem(NativeUiSized("AALS_GroupNoneButton"));
		GroupSoloButton = (CButtonWnd*)GetChildItem(NativeUiSized("AALS_GroupSoloButton"));
		GroupMasterButton = (CButtonWnd*)GetChildItem(NativeUiSized("AALS_GroupMasterButton"));
		GroupRobinButton = (CButtonWnd*)GetChildItem(NativeUiSized("AALS_GroupRobinButton"));
		GroupKillerButton = (CButtonWnd*)GetChildItem(NativeUiSized("AALS_GroupKillerButton"));
		UiSizeButton = (CButtonWnd*)GetChildItem(NativeUiSized("AALS_UiSizeButton"));
		UpdateUiSizeLabel();
	...
	int WndNotification(CXWnd* pWnd, unsigned int Message, void* unknown)
	{
		...
		if (Message == XWM_LCLICK) {
			...
			if (pWnd == (CXWnd*)UiSizeButton) {
				// Cycle S -> M -> L, persist, and let the pulse rebuild the
				// windows: this notification runs inside the window we are
				// about to delete, so the recreate must be deferred.
				gNativeUiScale = gNativeUiScale == kNativeUiScaleSmall ? kNativeUiScaleMedium :
					gNativeUiScale == kNativeUiScaleMedium ? kNativeUiScaleLarge : kNativeUiScaleSmall;
				NativeUiScaleSave();
				gNativeUiScalePendingRecreate = true;
				SetStatus("Switching UI size...");
				return 1;
			}
	...
	void UpdateUiSizeLabel()
	{
		if (UiSizeButton) {
			char text[48];
			sprintf_s(text, "UI Size: %s", NativeUiScaleLabel());
			CXStr value(text);
			((CXWnd*)UiSizeButton)->SetWindowTextA(value);
		}
	}
...
private:
	...
	CButtonWnd* UiSizeButton = nullptr;
};
...
{
public:
	NativeAutoLootManageWnd() : CCustomWnd((char*)NativeUiSized("NativeAutoLootManageWnd"))
	{
		CloseOnESC = 1;
		SetWndNotification(NativeAutoLootManageWnd);

		SummaryLabel = GetChildItem(NativeUiSized("AALM_SummaryLabel"));
		StatusLabel = GetChildItem(NativeUiSized("AALM_StatusLabel"));
		PlayerList = (CListWnd*)GetChildItem(NativeUiSized("AALM_PlayerList"));
		RefreshButton = (CButtonWnd*)GetChildItem(NativeUiSized("AALM_RefreshButton"));
		AskButton = (CButtonWnd*)GetChildItem(NativeUiSized("AALM_AskButton"));
		RollButton = (CButtonWnd*)GetChildItem(NativeUiSized("AALM_RollButton"));
		FreeGrabButton = (CButtonWnd*)GetChildItem(NativeUiSized("AALM_FreeGrabButton"));
		GiveButton = (CButtonWnd*)GetChildItem(NativeUiSized("AALM_GiveButton"));
		LeaveButton = (CButtonWnd*)GetChildItem(NativeUiSized("AALM_LeaveButton"));
		SetMasterButton = (CButtonWnd*)GetChildItem(NativeUiSized("AALM_SetMasterButton"));
		ClearMasterButton = (CButtonWnd*)GetChildItem(NativeUiSized("AALM_ClearMasterButton"));
...
{
public:
	NativeAutoLootMenuWnd() : CCustomWnd((char*)NativeUiSized("NativeAutoLootMenuWnd"))
	{
		CloseOnESC = 1;
		SetWndNotification(NativeAutoLootMenuWnd);
		ActionList = (CListWnd*)GetChildItem(NativeUiSized("AALM_List"));
		pXWnd()->Show(0, 1);
	}
	...
	void PopupForRow(bool shared, int entry_id, int anchor_x, int anchor_y)
	{
		...
		__try {
			const DWORD menu_row_height = (DWORD)NativeUiScaled(24);
			if (((DWORD*)ActionList)[0x21C / 4] != menu_row_height) {
				((DWORD*)ActionList)[0x21C / 4] = menu_row_height;
			}
		}
		__except (EXCEPTION_EXECUTE_HANDLER) {
		...
		const int width = NativeUiScaled(220);
		const int height = NativeUiScaled(156);
...
void NativeAutoLootRulesWnd::Layout()
{
	...
	const int check_size = NativeUiScaled(24);
	const int wide_size = NativeUiScaled(26);
	...
	for (int row = 0; row < RowCount && row < display_rows; ++row) {
		...
		CXRect target;
		CXRect clip;
		if (!NativeAutoLootCellControlRect(RuleList, row, columns_for_slot[0], check_size, false, &target, &clip)) {
			continue;
		}
		...
		for (int slot = 0; slot < 4; ++slot) {
			...
			if (!NativeAutoLootCellControlRect(RuleList, row, columns_for_slot[slot], check_size, false, &target, &clip)) {
				continue;
			}
			...
			raw->Location.right = new_left + check_size;
			raw->Location.bottom = new_top + check_size;
		...
		if (remove_button && NativeAutoLootCellControlRect(RuleList, row, 6, wide_size, false, &target, &clip)) {
			...
			raw->Location.left = new_left;
			raw->Location.top = new_top;
			raw->Location.right = new_left + wide_size;
			raw->Location.bottom = new_top + wide_size;
			if (moved) {
		...
		if (icon_button && row < (int)RowIconIds.size() &&
			NativeAutoLootCellControlRect(RuleList, row, 0, wide_size, true, &target, &clip)) {
...
bool NativeAutoLootWnd::GetIconCellDrawRect(CListWnd* list, int row, CXRect* target, CXRect* clip) const
{
	...
	__try {
		...
		const int control_size = NativeUiScaled(26);
		int vertical_inset = (bottom - top - control_size) / 2;
		if (vertical_inset < 0) {
			vertical_inset = 0;
		}
...
// Tears down just the Advanced Loot window family so it can be rebuilt with
// a different UI size preset (the screen/child names change with the suffix).
static void NativeAutoLootDestroyScaleWindows()
{
#define NATIVE_AUTOLOOT_DESTROY_WINDOW(ptr, label) \
	do { \
		if (ptr) { \
			...
			__except (EXCEPTION_EXECUTE_HANDLER) { \
				NativeAutoLootTrace("%s window release faulted during UI size switch", label); \
			} \
		} \
	} while (0)
	...
	NATIVE_AUTOLOOT_DESTROY_WINDOW(gNativeAutoLootSettingsWnd, "AutoLoot settings");
	NATIVE_AUTOLOOT_DESTROY_WINDOW(gNativeAutoLootManageWnd, "AutoLoot manage");
	NATIVE_AUTOLOOT_DESTROY_WINDOW(gNativeAutoLootMenuWnd, "AutoLoot menu");
...
static void NativeAutoLootPulse()
{
	...
	// Deferred UI-size switch: the Loot Settings button only flags the
	// change because the click notification runs inside a window we are
	// about to delete. Rebuild here, outside any window call stack.
	if (gNativeUiScalePendingRecreate) {
		gNativeUiScalePendingRecreate = false;
		const bool main_was_shown = gNativeAutoLootWnd && ((PCSIDLWND)gNativeAutoLootWnd->pXWnd())->dShow;
		const bool rules_was_shown = gNativeAutoLootRulesWnd && ((PCSIDLWND)gNativeAutoLootRulesWnd->pXWnd())->dShow;
		NativeAutoLootDestroyScaleWindows();
		NativeAutoLootShowSettingsWindow();
		if (main_was_shown) {
			NativeAutoLootSendCommand("/say #advloot window");
		}
		if (rules_was_shown) {
			NativeAutoLootShowRulesWindow();
		}
		NativeAutoLootTrace("UI size switched to %s", NativeUiScaleLabel());
	}
