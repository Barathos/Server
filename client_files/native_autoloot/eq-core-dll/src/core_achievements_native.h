#ifndef CORE_ACHIEVEMENTS_NATIVE_H
#define CORE_ACHIEVEMENTS_NATIVE_H
...
static bool gNativeAchievementsChatHookInstalled = false;
static bool gNativeAchievementsCommandHookInstalled = false;
...
static void NativeAchievementEnsureWindow(bool show)
{
	...
	if (!gNativeAchievementWnd) {
		...
		NativeAchievementsTrace("achievement window created");
	}
...
static bool NativeAchievementHandleLocalCommand(const char* line)
{
	if (!line) {
		return false;
	}
...
class NativeAchievementsChatHook
{
public:
	...
	{
		if (NativeAchievementParseTransport(szMsg)) {
			return;
		}

		if (szMsg && strstr(szMsg, "You say, '#ach")) {
			return;
		}
...
DETOUR_TRAMPOLINE_EMPTY(VOID NativeAchievementsChatHook::Trampoline(PCHAR szMsg, DWORD dwColor, bool EqLog, bool dopercentsubst));

class NativeAchievementsCommandHook
{
public:
	VOID Trampoline(EQPlayer* player, PCHAR line);
	VOID Detour(EQPlayer* player, PCHAR line)
	{
		if (NativeAchievementHandleLocalCommand(line)) {
			NativeAchievementsTrace("handled local achievement command: %s", line ? line : "");
			return;
		}

		char rewritten[256];
		if (NativeAchievementRewriteCommand(line, rewritten, sizeof(rewritten))) {
			NativeAchievementsTrace("rewrite command: %s -> %s", line ? line : "", rewritten);
...
DETOUR_TRAMPOLINE_EMPTY(VOID NativeAchievementsCommandHook::Trampoline(EQPlayer* player, PCHAR line));
...
static void NativeAchievementsInstallChatHook()
{
	if (gNativeAchievementsChatHookInstalled) {
		return;
	}

	NativeAchievementsTrace("installing achievement chat hook");
	EzDetour(CEverQuest__dsp_chat, &NativeAchievementsChatHook::Detour, &NativeAchievementsChatHook::Trampoline);
	gNativeAchievementsChatHookInstalled = true;
}

static void NativeAchievementsInstallCommandHook()
{
	if (gNativeAchievementsCommandHookInstalled) {
		return;
	}

	NativeAchievementsTrace("installing achievement slash command hook");
	EzDetour(CEverQuest__InterpretCmd, &NativeAchievementsCommandHook::Detour, &NativeAchievementsCommandHook::Trampoline);
	gNativeAchievementsCommandHookInstalled = true;
}
...
static void InitAchievementsNative()
{
	...
	NativeAchievementsInstallChatHook();
	NativeAchievementsInstallCommandHook();
...
static void ShutdownAchievementsNative()
{
	...
	if (gNativeAchievementsChatHookInstalled) {
		...
		gNativeAchievementsChatHookInstalled = false;
	}

	if (gNativeAchievementsCommandHookInstalled) {
		RemoveDetour(CEverQuest__InterpretCmd);
		gNativeAchievementsCommandHookInstalled = false;
	}
